...
# MAINTAINER Tim Hall (time@oracle-base.com)
LABEL maintainer="tim@oracle-base.com"

# ---------------------------------------------------------------------------------------------------------------------
# Define fixed (build time) environment variable.
...
# ---------------------------------------------------------------------------------------------------------------------
# Define config (runtime) environment variable.
ENV ORACLE_SID="cdb1" \
...
#
# Manually create user and group as preinstall package creates the with high IDs, which causes issues.
# Note 2 on link below.
# http://docs.docker.com/engine/userguide/eng-image/dockerfile_best_practices/#user
#
...
RUN ${ORACLE_HOME}/runInstaller -ignorePrereq \
    ...
    SECUTITY_UPDATES_VIA_MYORACLESUPPORT=false \
...
# ---------------------------------------------------------------------------------------------------------------------
# The start script perform all config based on runtime environment variables.
